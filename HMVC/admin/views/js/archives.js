...
function getinfo(page)
{
	...
	if(page==""||page==null)
		page=1;
	$.post("/admin/arclist/ajaxlist",{page:page,list:list,addtable:'allarc'},function(data){
		...
		for(i=0;i<row.length;i++)
			str+="<tr><td><input type='checkbox' value='"+row[i].id+"' id='check'/></td><td><a href='/archives/allarc/"+row[i].id+"' target='_blank'>"+row[i].title+"</a> &nbsp;&nbsp;<img src='/admin/views/images/headtopic_"+row[i].flag+".gif' /></td><td>"+row[i].short+"</td><td>"+row[i].update+"</td><td>"+row[i].writer+"</td><td><a href='/admin/arclist/edit/"+row[i].addtable+"/"+row[i].id+"' id='edit'><img src='/admin/views/images/icons/pencil.png' alt='Edit' /></a> <a href='#' title='Delete' id='del'><img src='/admin/views/images/icons/cross.png' alt='Delete' /></a></td></tr>";
		$("#rowinfo").html(str);
		nextfun();
	},"json");
}
