...
function getinfo(page)
{
	if(page==""||page==null)
		page=1;
	$.post("/admin/index/ajaxarctype",{page:page},function(data){
		...
		for(i=0;i<row.length;i++)
			str+="<tr><td><input type='checkbox' value='"+row[i].id+"' id='check'/></td><td>"+row[i].id+"</td><td>"+row[i].name+"</td><td>"+row[i].reid+"</td><td>"+row[i].typename+"</td><td>"+row[i].seotitle+"</td><td><a href='/admin/change/arctype/"+row[i].id+"' id='edit'><img src='/images/admin/icons/pencil.png' alt='Edit' /></a> <a href='#' title='Delete' id='del'><img src='/images/admin/icons/cross.png' alt='Delete' /></a></td></tr>";
		$("#rowinfo").html(str);
		nextfun();
	},"json");
}
